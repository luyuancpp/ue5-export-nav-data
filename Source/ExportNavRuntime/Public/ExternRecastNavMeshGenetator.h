...
public:
	...
	void GrowConvexHull(const dtReal ExpandBy, const TArray<FVector>& Verts, TArray<FVector>& OutResult);
	void TransformVertexSoupToRecast(const TArray<FVector>& VertexSoup, TNavStatArray<FVector>& Verts, TNavStatArray<int32>& Faces);
	void ExportGeomToOBJFile(const FString& InFileName, const TNavStatArray<dtReal>& GeomCoords, const TNavStatArray<int32>& GeomFaces, const FString& AdditionalData);
